...
impl EquipModel {
    ...
    pub fn is_signature_weapon(&self, uid: i32, avatar_id: i32) -> bool {
        self.weapons
            .get(&uid)
            .map_or(
                false,
                |weapon| self.context.filecfg.weapon_template_tb
                    .data()
                    .unwrap()
                    .iter()
                    .find(|tmpl| tmpl.item_id() == weapon.weapon_id)
                    .map_or(
                        false,
                        |tmpl| tmpl.avatar_id() == avatar_id
                    )
            )
    }
