...
@Command(label = "lineup", permission = "player.lineup", requireTarget = true, desc = "/lineup [角色ID]。风险自负。使用指定的角色ID设置你的当前阵容")
public class LineupCommand implements CommandHandler {
    ...
    public void execute(CommandArgs args) {
        ...
        if (avatars.size() > 0) {
            ...
            args.sendMessage("成功设置阵容给 " + target.getName() + " 成功");
        } else {
            args.sendMessage("没有角色可以添加");
        }
    }
