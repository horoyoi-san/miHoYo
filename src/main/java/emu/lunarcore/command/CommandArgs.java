    ...
    public CommandArgs(Player sender, List<String> args) {
        ...
        if (this.targetUid != 0) {
            if (LunarCore.getGameServer() != null) {
                this.onlineTarget = LunarCore.getGameServer().getOnlinePlayerByUid(this.targetUid);
                this.target = LunarCore.getGameServer().getPlayerByUid(this.targetUid, true);
            }
            } else {
            this.onlineTarget = sender;
            this.target = sender;
            if (sender != null) {
                this.targetUid = sender.getUid();
            }
        }
