...
public class SceneEntityLoader {
    ...
    public void onSceneLoad(Scene scene) {
        for (GroupInfo group : scene.getFloorInfo().getGroups().values()) {
            ...
            if (group.getGroupName().contains("DeployPuzzle_Repeat_Area")) {
                continue;
            }
    ...
    // 不加载NPCID小于1993的NPC，但排除ID为1003，1307的情况
    if (npcInfo.getNPCID() < 1993 && npcInfo.getNPCID() != 1003 && npcInfo.getNPCID() != 1307) {
        return null;
    }
    
    // 不生成重复的NPC
    if (scene.getEntities().values().stream()
            .anyMatch(entity -> entity instanceof EntityNpc && ((EntityNpc) entity).getNpcId() == npcInfo.getNPCID())) {
        return null;
    }
