...
import emu.lunarcore.server.packet.send.PacketEraFlipperDataChangeScNotify;
...
public class Scene implements Tickable {
    ...
    public void syncLineup() {
        ...
        // Sync packet
        getPlayer().sendPacket(new PacketSceneGroupRefreshScNotify(toAdd, toRemove, floorId));
    }
    
    public void getFloorIdChange(EraFlipperDataTypeInfo data) {
        getPlayer().sendPacket(new PacketEraFlipperDataChangeScNotify(data, floorId));
    }
    ...
    public synchronized void addEntity(GameEntity entity, boolean sendPacket) {
        ...
        // Send packet
        if (sendPacket) {
            player.sendPacket(new PacketSceneGroupRefreshScNotify(entity, null, floorId));
        }
    }
    ...
    public synchronized void removeEntity(int entityId) {
        ...
        if (entity != null) {
            ...
            entity.onRemove();
            // Send packet
            player.sendPacket(new PacketSceneGroupRefreshScNotify(null, entity, floorId));
            // Reset entity id
    ...
    public Map<Integer, List<SceneMarkedChestData>> MarkedChestData = new HashMap<>();
	public List<SceneMarkedChestData>[] MarkedChestDataList = new List[102];
	
	@Setter
	@Getter
	public static class SceneMarkedChestData {
        ...
        public MarkChestInfo ToProto() {
            var proto = MarkChestInfo.newInstance()
