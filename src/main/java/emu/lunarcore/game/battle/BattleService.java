...
public class BattleService extends BaseGameService {
    ...
    public void startBattle(Player player, int casterId, int attackedGroupId, MazeSkill castedSkill, IntSet hitTargets, IntSet assistMonsters) {
        ...
        // Check if we are using a skill that doesnt trigger a battle佩拉
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 1106) {
        ...
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 1217) {
        // Apply buffs to monsters
        castedSkill.onCastHit(player.getCurrentLeaderAvatar(), targets);
        // Skip battle if our technique does not trigger a battle
        player.sendPacket(new PacketSceneCastSkillScRsp(attackedGroupId));
        ...
        // Check if we are using a skill that doesnt trigger a battle
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 1220) {
        // Apply buffs to monsters
        castedSkill.onCastHit(player.getCurrentLeaderAvatar(), targets);
        // Skip battle if our technique does not trigger a battle
        player.sendPacket(new PacketSceneCastSkillScRsp(attackedGroupId));
        ...
        // Check if we are using a skill that doesnt trigger a battle
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 1308) {
        // Apply buffs to monsters
        castedSkill.onCastHit(player.getCurrentLeaderAvatar(), targets);
        // Skip battle if our technique does not trigger a battle
        player.sendPacket(new PacketSceneCastSkillScRsp(attackedGroupId));
        ...
        // Check if we are using a skill that doesnt trigger a battle
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 1224) {
        // Apply buffs to monsters
        castedSkill.onCastHit(player.getCurrentLeaderAvatar(), targets);
        // Skip battle if our technique does not trigger a battle
        player.sendPacket(new PacketSceneCastSkillScRsp(attackedGroupId));
        ...
        // Check if we are using a skill that doesnt trigger a battle
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 1317) {
        // Apply buffs to monsters
        castedSkill.onCastHit(player.getCurrentLeaderAvatar(), targets);
        // Skip battle if our technique does not trigger a battle
        player.sendPacket(new PacketSceneCastSkillScRsp(attackedGroupId));
        ...
        // Check if we are using a skill that doesnt trigger a battle
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 1402) {
        // Apply buffs to monsters
        castedSkill.onCastHit(player.getCurrentLeaderAvatar(), targets);
        // Skip battle if our technique does not trigger a battle
        player.sendPacket(new PacketSceneCastSkillScRsp(attackedGroupId));
        ...
        // Check if we are using a skill that doesnt trigger a battle
        if (castedSkill != null && !castedSkill.isTriggerBattle() && castingAvatar.getAvatarId() != 8008) {
        // Apply buffs to monsters
        castedSkill.onCastHit(player.getCurrentLeaderAvatar(), targets);
        // Skip battle if our technique does not trigger a battle
        player.sendPacket(new PacketSceneCastSkillScRsp(attackedGroupId));
    ...
    public void QuickstartCocoon(Player player, int cocoonId, int worldLevel, int wave) {
        // Sanity check to make sure player isnt in a battle
        if (player.isInBattle()) {
            return;
        }
        ...
        CocoonExcel cocoonExcel = GameData.getCocoonExcel(cocoonId, worldLevel);
        if (cocoonExcel == null) {
            player.sendPacket(new PacketQuickStartCocoonStageScRsp());
            return;
        }
        ...
        // Send packet
        player.sendPacket(new PacketQuickStartCocoonStageScRsp(battle, cocoonId, wave));
    }
