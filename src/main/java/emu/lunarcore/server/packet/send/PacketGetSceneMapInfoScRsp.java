...
public class PacketGetSceneMapInfoScRsp extends BasePacket {
    ...
    public PacketGetSceneMapInfoScRsp(RepeatedInt list, RepeatedInt floorMissionList) {
        super(CmdId.GetSceneMapInfoScRsp);

        var data = GetSceneMapInfoScRsp.newInstance();

        for (int floor : floorMissionList) {
...
/*
            // Get the default emtramce
            var defaultEntrance = GameData.getMapDefaultEnteranceExcelMap().get(floorId);
            if (defaultEntrance == null) {
                LunarCore.getLogger().error("[GetSceneMapInfoScRsp] defaultEntrance is NULL!");
                data.addMapList(mazeMap);
                continue;
            }
            
            // Get map entrance excel
            MapEntranceExcel excel = GameData.getMapEntranceExcelMap().get(defaultEntrance.getEntranceID());
            if (excel == null) {
                LunarCore.getLogger().error("[GetSceneMapInfoScRsp] dMap Entrance is NULL!");
                data.addMapList(mazeMap);
                continue;
            }
            
            mazeMap.setFloorId(floorInfo.getfloorID());
*/ 